...
# Create an EC2 instance with an Ubuntu AMI (or another Linux distribution)
resource "aws_instance" "portal_ec2" {
  ...
  # Provisioners to copy files and run the container
  provisioner "file" {
  ...
  provisioner "remote-exec" {
    ...
    connection {
      ...
      host = aws_instance.portal_ec2.public_ip
    }
  }
}
